...
import type { Incident, IncidentStatus, RiskBand } from "@/lib/types";
...
export default function IncidentsPage() {
  ...
  useEffect(() => { api.getIncidents().then(setIncidents).catch(() => undefined); }, []);

  const filtered = useMemo(() => incidents.filter((incident) => {
    const matchesStatus = status === "all" || incident.status === status;
    const matchesBand = band === "all" || incident.band === band;
    const normalized = query.toLowerCase();
    return matchesStatus && matchesBand && (!normalized || incident.endpoint.toLowerCase().includes(normalized) || incident.session_key.toLowerCase().includes(normalized));
  }), [band, incidents, query, status]);

  return <div className="space-y-6">
    <header className="flex items-end justify-between gap-4"><div><p className="eyebrow">Detection feed</p><h1 className="page-title">Incidents <span className="ml-2 text-base font-normal text-zinc-500">{filtered.length} visible</span></h1></div><Mono className="hidden text-xs text-zinc-600 sm:block">FIXTURE MODE</Mono></header>
    ...
    <div className="console-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-205 text-left text-sm"><thead className="border-b border-zinc-800 bg-zinc-950/50 text-[11px] uppercase tracking-[0.08em] text-zinc-600"><tr><th className="px-4 py-3 font-medium">Time</th><th className="px-4 py-3 font-medium">Threat</th><th className="px-4 py-3 font-medium">Endpoint</th><th className="px-4 py-3 font-medium">Session</th><th className="px-4 py-3 font-medium">Score</th><th className="px-4 py-3 font-medium">Ladder</th><th className="px-4 py-3 font-medium">Status</th></tr></thead><tbody className="divide-y divide-zinc-800/70">{filtered.map((incident) => <IncidentRow key={incident.id} incident={incident} />)}</tbody></table></div>{filtered.length === 0 && <div className="px-6 py-16 text-center"><p className="text-sm text-zinc-400">Nothing flagged for these filters.</p><p className="mt-1 text-xs text-zinc-600">Monika is watching 9 endpoints.</p></div>}</div>
  </div>;
}

function IncidentRow({ incident }: { incident: Incident }) {
  const ladder = incident.actions.at(-1)?.state ?? "NORMAL";
  return <tr className="group transition-colors hover:bg-zinc-900/70"><td className="whitespace-nowrap px-4 py-3"><Link href={`/incidents/${incident.id}`} className="mono text-xs text-zinc-500 group-hover:text-indigo-300">{new Date(incident.last_seen).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</Link></td><td className="px-4 py-3"><ThreatTypeLabel threatType={incident.threat_type} /></td><td className="px-4 py-3"><Mono className="text-xs text-zinc-400">{incident.method} {incident.endpoint}</Mono></td><td className="max-w-36 truncate px-4 py-3"><Mono className="text-xs text-zinc-600">{incident.session_key}</Mono></td><td className="px-4 py-3"><ScoreBadge score={incident.score} band={incident.band} /></td><td className="px-4 py-3"><LadderBadge state={ladder} /></td><td className="px-4 py-3"><div className="flex items-center gap-2"><BandBadge band={incident.band} /><span className="text-xs capitalize text-zinc-500">{incident.status}</span></div></td></tr>;
}
