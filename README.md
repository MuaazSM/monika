...
Open the dashboard on port 3000. With benign traffic running you'll see a quiet overview: precision 1.00, request tiles ticking, no incidents. Fire `make demo s=idor` and within three requests a **BOLA_ENUMERATION** incident appears with its score climbing into the 90s–100. Click it to see the full evidence list (jwt_sub, foreign object owner, sensitive field present, sequential IDs, z-score) and a **RATE_LIMIT → CHALLENGE → BLOCK → REVOKE** timeline. A moment later the LLM explanation is written back onto the incident — after the decision was already enforced. Keep sweeping and the attacker's requests turn to 403s while benign traffic is untouched; an analyst can mark the incident a false positive and watch the ladder drop to NORMAL within one request, leaving an immutable override row in the audit trail.
